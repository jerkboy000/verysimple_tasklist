...
const resolvers = {
  Query: {
    ...
    getTasksByUser: async (_, { user_id }) => {
      try {
        ...
        logger.error(
          `Error retrieving tasks by user ID ${user_id}: ${error.message}`,
        );
        throw error;
      }
    },
  ...
  Mutation: {
    ...
    createTask: async (
      _,
      { user_id, name, description, note, due_date, status },
    ) => {
      try {
    ...
    updateTask: async (
      _,
      { id, name, description, note, due_date, status },
    ) => {
      try {
        await Task.update(
          { name, description, note, due_date, status },
          { where: { id } },
        );
